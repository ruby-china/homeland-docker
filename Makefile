...
install_ssl:
	...
	$(RUN_WEB) bash -c '$(ACME) --issue -d $$domain -w /home/app/homeland/public $(ACME_HOME) --debug'
	openssl dhparam -out shared/ssl/dhparam.pem 2048
	$(RUN_WEB) bash -c '$(ACME) --installcert $(ACME_HOME) -d $$domain --keypath /home/app/ssl/homeland.key --fullchainpath home/app/ssl/homeland.crt --reloadcmd "nginx -s reload"'
